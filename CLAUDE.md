...
- **Stow packages**: `bash`, `vim`, `tmux`, `git`, `starship`, `kitty`, `gh`, `claude` — defined in the `PACKAGES` array in `install.sh` and `uninstall.sh`. Add new packages to both scripts. The `kitty` package contains a `kitty-themes` git submodule (`dexpota/kitty-themes`) — clone with `--recurse-submodules` or run `git submodule update --init`.
